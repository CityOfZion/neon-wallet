...
export const WalletsPage = () => {
  ...
  const selectedAccounts = useMemo(() => {
    return selectedAccount ? [selectedAccount] : accounts.filter(account => account.idWallet === selectedWallet?.id)
  }, [accounts, selectedWallet, selectedAccount])

  const balanceExchange = useBalancesAndExchange(accounts)
  const selectedAccountsBalanceExchange = useBalancesAndExchange(selectedAccounts)
  ...
  return (
    ...
    >
      ...
      <div className="flex-grow flex flex-col gap-y-3 min-w-0">
        <BalanceChartPanel
          balanceExchange={selectedAccountsBalanceExchange}
          accounts={selectedAccounts}
          wallets={wallets}
        />
        <BlockchainDataPanel balanceExchange={selectedAccountsBalanceExchange} accounts={selectedAccounts} />
      </div>
    </MainLayout>
  )
}
